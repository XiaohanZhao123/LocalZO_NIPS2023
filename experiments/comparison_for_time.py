...
def main(cfg):
    ...
    print(f'dataset setup: batch_size{cfg.dataset.batch_size}, num_step:{cfg.dataset.num_step}')
    lr = cfg.optimizer.lr
    ...
    with open(f'/home/zxh/remote/sparse_zo_nips2023/experiments/results/{dataset_name}_{model_name}_snntorch_time.txt',
              'a+') as f:
        f.write(
            f'batch_size:{cfg.dataset.batch_size}, num_step:{cfg.dataset.num_step}, snn_torch_mean_time:{snntorch_mean_time}, spconv_mean_time{spconv_mean_time}\n')
    torch.cuda.empty_cache()
